...
> 挂在 dsh-trivium 上的**情节层**：把 DSH 已有的 compaction 检查点与 fork 谱系画成横向方框图，顶上用可选记忆芯片即插即用。  
> 状态：**已随 dsh-trivium@0.4.0 发布**；**0.4.1** 把宿主钉到 `0.1.0-rc.8`；**0.4.2** 芯片条可新增、批量归档 / 删除；**0.4.3** 旧会话「更新检查点」；**0.4.4** 接 TriviumDB 0.7.5 入边 / label 扩邻。第一刀 / 第二刀 / 第三刀完成。内核仍是四工具 + 短地图；本面不替代压缩，不新建聊天窗口。  
> 宿主：`@deepseek-ai/dsh@0.1.0-rc.8`（必须钉死）。  
> 主规划仍是 [`PLAN.md`](./PLAN.md)。本文只覆盖会话图；与主规划冲突时以本文这一面为准，内核规则仍听主规划。

---

## 1. 一句话

同一会话窗口里，compaction 切一段就收成一个方框；人在我们的画布上点方框去 fork；勾选的记忆芯片从**当前这段的下一轮**起注入。压缩仍由 DSH 做，我们负责看见、钉住、分得出去。
...
- **主入口**：会话标题栏「对话 / 轨迹」旁的第三个标签 **会话图**（`conversation.view`，`id: session-map`，`order: 20`）。
...
钉 `@deepseek-ai/dsh@0.1.0-rc.8`。同一 `.tdb` 不要被两个 Node 进程同时打开。
