...
> 状态：**已随 dsh-trivium@0.4.0 发布**。第一刀 / 第二刀 / 第三刀完成。内核仍是四工具 + 短地图；本面不替代压缩，不新建聊天窗口。  
> 宿主：`@deepseek-ai/dsh@0.1.0-rc.6`（必须钉死）。  
...
钉 `@deepseek-ai/dsh@0.1.0-rc.6`。同一 `.tdb` 不要被两个 Node 进程同时打开。
