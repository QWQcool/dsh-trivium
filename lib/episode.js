...
/**
 * Cut the live tail into a historical box at `atSeq`, then keep one 「后续」.
 * Same atSeq reuses the checkpoint (no extra box). Does not compact DSH context.
 */
export function cutSessionMap(db, { sessionId, atSeq, summary } = {}) {
  const sid = String(sessionId || "");
  const seq = Number(atSeq);
  if (!sid) return { ok: false, message: "need sessionId" };
  if (!Number.isFinite(seq) || seq < 0) return { ok: false, message: "need atSeq" };
  const existing = findByUri(db, episodeUri(sid, seq));
  const existed = existing != null && isCheckpointPayload(db.get(existing)?.payload);
  if (existed) {
    return {
      ok: true,
      created: false,
      ...
      atSeq: seq,
      ...sessionMapSnapshot(db, sid),
    };
  }
  const id = recordCheckpoint(db, {
    sessionId: sid,
    atSeq: seq,
    compactionId: `manual-${seq}`,
    summary: String(summary || "").trim() || "检查点",
  });
  return {
    ok: true,
    created: true,
...
function isCheckpointPayload(payload) {
  return payload?.type === "episode" && payload.kind === "checkpoint";
}

function parentCutId(db, parent, seq) {
  if (seq != null) {
    const existing = findByUri(db, episodeUri(parent, seq));
    if (existing != null && isCheckpointPayload(db.get(existing)?.payload)) return existing;
  }
  const parentRows = listEpisodeRecords(db, parent);
  if (seq != null) {
    const hit = [...parentRows]
      .reverse()
      .find((e) => e.kind === "checkpoint" && (e.atSeq || 0) <= seq);
    if (hit) return hit.id;
  }
  const lastCheck = [...parentRows].reverse().find((e) => e.kind === "checkpoint");
  return lastCheck?.id ?? parentRows.find((e) => e.kind === "tail")?.id ?? null;
}

/**
 * Project a native or canvas fork onto the graph.
 * Branch from an existing checkpoint or the parent's tail — never insert a dummy cut box.
 */
export function syncForkLineage(db, { childSessionId, parentSessionId, atSeq } = {}) {
  ...
  const cutId = parentCutId(db, parent, seq);
...
/**
 * Project DSH's already-landed compactations and fork list onto episode boxes.
 * Idempotent: same atSeq / child reuses the existing node.
 */
export function backfillSessionMap(db, { sessionId, compactations = [], forks = [] } = {}) {
  const sid = String(sessionId || "");
  let checkpoints = 0;
  const seenSeq = new Set();
  for (const raw of compactations || []) {
    const atSeq = Number(raw.atSeq);
    if (!sid || !Number.isFinite(atSeq) || seenSeq.has(atSeq)) continue;
    seenSeq.add(atSeq);
    const id = recordCheckpoint(db, {
      sessionId: sid,
      atSeq,
    ...
    if (id != null) checkpoints += 1;
  }
  if (sid) ensureTail(db, sid);
  let forkLinks = 0;
  for (const raw of forks || []) {
    const child = String(raw.childSessionId || "");
    const parent = String(raw.parentSessionId || "");
    if (!child || !parent) continue;
    const atSeq = Number.isFinite(Number(raw.atSeq)) ? Number(raw.atSeq) : undefined;
    const result = syncForkLineage(db, {
      childSessionId: child,
      parentSessionId: parent,
      atSeq,
    });
    if (result?.childTail != null) forkLinks += 1;
  }
  return {
    ok: true,
    checkpoints,
