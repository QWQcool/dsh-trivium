...
import { detectDshEmbedding, dshApiCredentials, embeddingPublicStatus } from "./embed.js";
...
async function handle(req, res, { liveOptions, status }) {
  ...
  if (path === "/api/dsh-trivium/embed-detect" && method === "POST") {
    const detected = await detectDshEmbedding();
    if (detected.foundKey && detected.embeddingUrl) {
      writeUiSettings({
        embeddingEnabled: detected.ok === true,
        embeddingUrl: detected.embeddingUrl,
        embeddingModel: detected.embeddingModel,
        embeddingApiKey: dshApiCredentials().apiKey,
      });
    }
    const opts = liveOptions();
    sendJson(res, 200, {
      ok: true,
      ready: detected.ok === true,
      foundKey: detected.foundKey,
      probed: detected.probed,
      message: detected.message,
      embeddingDim: detected.embeddingDim,
      dimOk: detected.dimOk,
      recallMode: recallModeOf(opts),
      ...embeddingPublicStatus(),
    });
    return;
