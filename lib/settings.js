...
/** This file can hold an embedding API key; keep it owner-only. No-op on Windows. */
...
function persistSettings(next) {
  // Atomic: a truncated settings file would make readUiSettings() return {}
  // and silently reset every switch / chip pin.
  atomicWriteFile(settingsFilePath(), JSON.stringify(next, null, 2), { mode: SETTINGS_FILE_MODE });
  return next;
}
