...
function linkNameOf(span, turnText, uniqueTurn) {
  return (
    collectNames(span, 1)[0]?.name ||
  ...
  for (const turn of turns || []) {
    ...
    for (const span of prefSpans) {
      ...
      out.push({
        ...
        text: clip(span, 240),
        quote: clip(span, 120),
        linkName: linkNameOf(span, text, uniqueTurn),
    ...
    for (const span of decisionSpans) {
      ...
      const object = linkNameOf(span, text, uniqueTurn);
      out.push({
        type: "decision",
        name: object,
        text: clip(span, 240),
