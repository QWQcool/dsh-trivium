  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        chipTab: "芯片",
        chipsSetting: "芯片标签（默认关）",
        chipsSettingHint: "打开后，标题栏「对话 / 轨迹」旁出现「芯片」。勾选的条目从下一轮注入（L0，≤300 token）。节点仍在 .tdb，关开关不删记忆。改完保存后重启 dsh web，标签才会出现或消失。",
      ...
      en: {
        ...
        chipTab: "Chips",
        chipsSetting: "Chips tab (off by default)",
        chipsSettingHint: "When on, the title bar shows Chips next to Conversation / Trajectory. Checked items inject from the next turn (L0, ≤300 tokens). Nodes stay in .tdb; turning this off does not delete memory. Restart dsh web after save for the tab to appear or disappear.",
              ...
              jsxs("label", {
                ...
                children: [
                  jsx("input", {
                    ...
                    onChange: (e) => setChipsEnabled(e.target.checked),
                  }),
              ...
              jsxs("label", {
                ...
                children: [
                  jsx("input", {
                    ...
                    onChange: (e) => setSessionLayerEnabled(e.target.checked),
                  }),
    ...
    function apply(ctx) {
      ...
      fetch("/api/dsh-trivium/status")
        .then((resp) => (resp && resp.ok ? resp.json() : null))
        .then((st) => {
          if (st && st.chipsEnabled === true) registerChipView(ctx, sessions);
        })
        .catch(() => {
          // default off: no title-bar tab
        });
    }
