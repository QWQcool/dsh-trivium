window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const L = {
      ...
      workbuddy: "WorkBuddy 一次性导入",
      workbuddyHint: "只读 ~/.workbuddy/MEMORY.md 与工作区 .workbuddy/memory/MEMORY.md。偏严宁漏：闲聊、一次性改文件、密钥、无规则线索的段落跳过。不持续同步。每批同样 ≤3000 字 / 24 条。",
      workbuddyImport: "导入找到的 MEMORY.md",
      workbuddyNone: "当前没有找到这些文件。",
      workbuddyFound: "找到",
    ...
    function TriviumCard() {
      ...
      const load = react.useCallback(async () => {
        ...
        try {
          ...
          setNodes(list.nodes || []);
          const wb = await api("/api/dsh-trivium/workbuddy");
      ...
      const importBuddy = async () => {
        ...
        setBuddyNote("");
        try {
          const r = await api("/api/dsh-trivium/import-workbuddy", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: "{}",
          });
        ...
        children: [
          ...
          jsxs("div", {
            ...
            children: [
              ...
              jsx("div", { style: ui.cardTitle, children: L.workbuddy }),
              jsx("div", { style: ui.hint, children: L.workbuddyHint }),
              jsx("div", {
    ...
    function kindLabel(kind, current) {
      if (current || kind === "tail") return "后续";
      if (kind === "fork") return "分支";
      return "检查点";
    }

    const GENERIC_BOX_NAMES = { "当前": 1, "已压缩": 1, "分叉": 1, "分叉点": 1, "后续": 1, "分支": 1 };
    const MAP_BOX_W = 220;
    const MAP_ARROW_W = 36;
    const MAP_PAD = 32;
    const MAP_BOX_H = 120;
    const MAP_KID_GAP = 10;
    const MAP_KID_H = 96;
    const MAP_COL_H = MAP_PAD * 2 + MAP_BOX_H + 2 * (MAP_KID_H + MAP_KID_GAP);
    ...
    function layoutChain(nodes, edges, sessionId) {
      const mine = (nodes || []).filter((n) => n.sessionId === sessionId);
      const checks = mine
        .filter((n) => n.kind !== "tail")
    ...
    function panToForkChild(nodes, edges, parentSessionId, childSessionId, canvas, scale) {
      const { chain, kids } = layoutChain(nodes, edges, parentSessionId);
      ...
      const aboveH = foundKid && split.above.length ? MAP_KID_H + MAP_KID_GAP : 0;
      const belowH = foundKid && split.below.length ? MAP_KID_H + MAP_KID_GAP : 0;
      const y = midY - aboveH;
    ...
    function ArrowH() {
      return jsxs("svg", {
        ...
        children: [
          ...
          jsx("polygon", { points: "22,3 33,9 22,15", fill: "currentColor" }),
        ],
      });
    }
    ...
    function ArrowV({ dir }) {
      ...
      return jsxs("svg", {
        ...
        children: [
          ...
          jsx("polygon", { points: "3,13 9,21 15,13", fill: "currentColor" }),
        ],
      });
    }
    ...
    const mapUi = {
      ...
      sliderWrap: { display: "flex", alignItems: "center", gap: 8, padding: "0 2px" },
      slider: { flex: 1, minWidth: 0, accentColor: "currentColor" },
      canvas: {
        flex: 1,
        minHeight: 260,
        overflow: "hidden",
    ...
    function EpisodeBox({ node, current, child, onJump, onFork, onInspect, onRename, canSetView, busy, livePreview }) {
        ...
        children: [
          jsxs("div", {
            ...
            children: [
              jsxs("div", {
                ...
                children: [
                  jsxs("div", {
                    ...
                    onDoubleClick: startEdit,
                    children: [
                      node.atSeq != null && Number(node.atSeq) > 0 ? "seq " + node.atSeq : kindLabel(node.kind, current),
                    ],
                  }),
                  canRename
                    ? jsx("button", {
              ...
              jsx("div", { style: mapUi.summary, children: body || "（尚无摘要）" }),
    ...
    function SessionMapView({ sessionId, useSessions, useSession, fork, open, actions }) {
      ...
      const lineageKey = useSessions((s) => {
        ...
        kids.sort();
        return String(byId[sessionId]?.parentSessionId || "") + ":" + kids.join(",");
      });
      const [nodes, setNodes] = react.useState([]);
      ...
      const { chain, kids } = react.useMemo(
        () => layoutChain(nodes, edges, sessionId),
      ...
      const openSession = (id, nav) => {
        if (typeof open !== "function") {
          setError("无法打开分叉会话：会话图没有拿到 session.open");
          return false;
        }
        try {
      ...
      const jumpToChat = (node) => {
        ...
        if (typeof actions?.setView !== "function") {
          setError("无法切换到对话页：会话图没有拿到同一 chat store 的 setView");
          return;
        }
      ...
      const jumpToTrajectory = (node) => {
        ...
        if (typeof actions?.setView === "function") {
          ...
          setError("无法切换到轨迹页：会话图没有拿到同一 chat store 的 setView");
        }
      };
      ...
      const forkBox = async (node) => {
        if (busy) return;
        setBusy(true);
        setError("");
        try {
          if (typeof fork !== "function") throw new Error("当前壳没有 session.fork");
      ...
      const sliderValue = (() => {
        const lo = minPanX(chain.length, canvasRef.current?.clientWidth || 640, scale);
        if (lo >= 0) return 0;
        return Math.max(0, Math.min(100, Math.round(((0 - pan.x) / (0 - lo)) * 100)));
      })();
      ...
      return jsxs("div", {
        ...
        children: [
          jsxs("div", {
            ...
            children: [
              jsx("div", { style: mapUi.chipsTitle, children: "记忆芯片" }),
              jsx("div", {
                style: mapUi.hint,
                children:
                  "共 " +
                  ...
                  " 条，已钉 " +
                  pinIds.length +
                  "。默认不勾选：不钉就不会按芯片白名单注入，但仍可能被开场短地图点名带上，模型也能 find 到。",
              }),
              jsx("button", {
                type: "button",
                style: mapUi.foldBtn,
                onClick: () => setChipsOpen((v) => !v),
                children: chipsOpen ? "收起" : "展开勾选",
              }),
            ],
          }),
          chipsOpen
                ...
                children: [
                  jsxs("div", {
                    ...
                    children: [
                      ...
                      clipped
                        ? jsx("div", { style: mapUi.hint, children: "已截到预算（≤300 token）" })
                        : jsx("div", { style: mapUi.hint, children: "勾选后从当前这段的下一轮注入。滚轮缩放，底栏滑条平移。" }),
                    ],
                  }),
                  visibleChips.length
                    ? jsx("div", {
                        ...
                        children: visibleChips.map((c) =>
                          jsxs(
                            "label",
                            {
                              style: { ...mapUi.chip, ...(c.suggested ? mapUi.chipSuggest : {}) },
                              children: [
                                jsx("input", {
                                  type: "checkbox",
                                ...
                                c.suggested ? jsx("span", { style: mapUi.suggest, children: "建议" }) : null,
                                jsx("span", {
                    ...
                    : jsx("div", {
                        ...
                        children: chips.length
                          ? "没有匹配的芯片。"
                          : "还没有可钉选的 preference / decision / entity。",
                      }),
                ],
              })
            : null,
                            ...
                            jsxs("div", {
                              ...
                              children: [
                                jsxs("div", {
                                  ...
                                  children: [
                                    split.above.map((kid) =>
                                      jsx(EpisodeBox, { node: kid, child: true, ...boxProps }, "k" + kid.id),
                                    ),
                                    split.above.length
          ...
          jsxs("div", {
            style: mapUi.sliderWrap,
            children: [
              jsx("span", { style: mapUi.hint, children: "链" }),
              jsx("input", {
                type: "range",
                min: 0,
                max: 100,
