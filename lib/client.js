window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function boxKicker(node, { current, child, sessionTitle }) {
      if (child) return sessionTitle || "分叉";
      if (current || node?.kind === "tail") return "后续";
    ...
    function SessionMapView({ sessionId, useSessions, useSession, fork, open, renameSession, actions, seedLengthOf }) {
      ...
      const syncCheckpoints = async () => {
        ...
        try {
          ...
          if (!checks && !forkN) {
            setBackfillNote("没有发现压缩标记或分叉。没压过的会话本来就只有「后续」。要分段请点「生成检查点」。");
          } else {
            setBackfillNote("已同步 " + checks + " 个检查点、" + forkN + " 条分叉。");
          }
        } catch (err) {
          setError(String(err.message || err));
        } finally {
      ...
      const cutCheckpoint = async () => {
        ...
        if (atSeq == null) {
          setError("当前对话还没有可切的轮次。先在对话里发一条，再生成检查点。");
          return;
        }
        setBusy(true);
        setError("");
        try {
          ...
          applyMap(resp);
          if (resp.created) {
            setBackfillNote("已把当前「后续」收成左边一格（seq " + atSeq + "），右边仍是后续。这不压缩 DSH 上下文。");
          } else {
            setBackfillNote("这个位置已经切过了（seq " + atSeq + "）。再聊几轮后再生成。");
          }
        } catch (err) {
          setError(String(err.message || err));
        } finally {
        ...
        try {
          ...
          if (childId && wanted && typeof renameSession === "function") {
            try {
              ...
              setError("会话已分叉，但侧边栏改名失败：" + String(err.message || err));
            }
          }
      ...
      const runChipBatch = async (action) => {
        ...
        if (action === "delete") {
          const ok = window.confirm(
            "删除会从 .tdb 里拿掉这 " +
              pickedIds.length +
              " 条，不可恢复。归档只是藏起来，find / 短地图 / 芯片都不再出现。确定删除？",
          );
          if (!ok) return;
        }
        setBusy(true);
              ...
              jsxs("div", {
                ...
                children: [
                  jsx("button", {
                    ...
                    disabled: busy,
                    title: "把当前「后续」收成左边一格，右边继续当后续。只切情节图，不触发 DSH 压缩。要压上下文请在对话里输入 /compact。",
                    onClick: cutCheckpoint,
                    children: busy ? "生成中…" : "生成检查点",
                  }),
                  jsx("button", {
                    type: "button",
                    style: mapUi.actionBtn,
                    disabled: busy,
                    title: "读取当前对话里 DSH 已有的压缩标记和侧边栏分叉，补进图。没压过就补不出历史方框。",
                    onClick: syncCheckpoints,
                    children: busy ? "更新中…" : "更新检查点",
                  }),
                  jsx("button", {
                    type: "button",
                    style: mapUi.actionBtn,
            ...
            ? jsxs("div", {
                ...
                children: [
                        ...
                        children: [
                          ...
                          chipSelect
                            ? jsx("button", {
                                ...
                                children: "归档" + (pickedIds.length ? " " + pickedIds.length : ""),
                              })
                            : null,
                          chipSelect
                            ? jsx("button", {
                                ...
                                children: "删除" + (pickedIds.length ? " " + pickedIds.length : ""),
                              })
                            : null,
                        ],
                      }),
                  ...
                  jsx("div", {
                    style: mapUi.hint,
                    children: chipSelect
                      ? "点芯片多选。归档：软删除，find / 短地图 / 芯片不再出现，节点还在库里。删除：从 .tdb 去掉，不可恢复。"
                      : clipped
                        ? "已截到预算（≤300 token）"
                        : "勾选后从当前这段的下一轮注入。空白处拖动画布，滚轮缩放。",
                  }),
                  draftOpen
                    ? jsxs("div", {
                        ...
                        children: [
                          jsx("textarea", {
                            ...
                            placeholder: "粘贴或输入要记住的内容。点一次写入，整段就是一条芯片。",
                  ...
                  visibleChips.length
                    ...
                    : jsx("div", {
                        ...
                        children: chips.length
                          ? "没有匹配的芯片。"
                          : "还没有可钉选的 preference / decision / entity。用「新增」写一条。",
                      }),
                ],
              })
            : null,
          ...
          forkDraft
            ? jsx("div", {
                ...
                children: jsxs("form", {
                  ...
                  children: [
                    jsx("div", { style: mapUi.dialogTitle, children: "分叉为新会话" }),
                    jsx("div", {
                      style: mapUi.hint,
                      children: "这个名字会出现在左侧会话列表里。",
                    }),
                    jsx("input", {
                      autoFocus: true,
              ...
              inject: () => ({
                ...
                renameSession: async (id, title) => {
                  ...
                  if (!face || typeof face.rename !== "function") {
                    throw new Error("当前壳没有 session.rename");
                  }
