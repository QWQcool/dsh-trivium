...
function textTool(definition) {
  return defineTool({
    ...
    presentCall: (args) => ({
      card: "generic",
      kind: definition.name === "ctx_find" || definition.name === "ctx_read" ? "read" : "other",
...
export function registerTools(ctx, options) {
  ...
  ctx.tools.register(
    textTool({
      name: "ctx_find",
      description:
        "Search local Trivium graph memory. Returns L0 summaries plus edge paths. Call ctx_read for full text.",
      parameters: {
        ...
        try {
          ...
          return hits
            .map((hit) => {
              ...
              return `[${row.score.toFixed(3)}] id=${row.id} type=${row.type}\n${row.l0}\npath: ${row.path.join(" | ") || "(none)"}`;
            })
            .join("\n\n");
        } catch (err) {
    ...
    textTool({
      ...
      async execute(args, exec) {
        try {
          ...
          const type = ["entity", "preference", "decision", "experience"].includes(args.type)
            ? args.type
            : "preference";
          const db = await dbOf(exec);
          const id = insertNode(db, {
            ...
            source: { sessionId: String(exec?.agent?.id || "") },
          });
          if (args.link_to) {
            const label = args.link_label || EDGE_LABELS.about;
            db.link(id, Number(args.link_to), label, 1);
            db.flush();
          }
    ...
    textTool({
      ...
      async execute(args, exec) {
        try {
          ...
          db.link(Number(args.from), Number(args.to), label, 1);
          db.flush();
