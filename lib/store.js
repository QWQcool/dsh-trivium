...
export function mergeNodes(db, keepId, dropId) {
  ...
  for (const other of db.allNodeIds?.() || []) {
    ...
    for (const e of n?.edges || []) {
      if (Number(e.targetId ?? e.target_id) === drop) {
        ensureLink(db, other, keep, e.label);
      }
    }
  }
...
export function exportGraph(db, cwd = "") {
  ...
  for (const id of db.allNodeIds?.() || []) {
    ...
    for (const e of node.edges || []) {
      edges.push({
        from: Number(id),
        to: Number(e.targetId ?? e.target_id),
        label: e.label || "",
...
/** Link once for a (from, to, label) triple. Engine unlink cannot drop a single label. */
...
function entityAnchorIds(db, query) {
  ...
  for (const id of db.allNodeIds?.() || []) {
    const node = db.get(id);
    if (node?.payload?.status === "archived" || node?.payload?.type === "episode") continue;
