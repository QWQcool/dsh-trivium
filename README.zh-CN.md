...
> **快速安装**：`dsh plugin --profile web add dsh-trivium` → 重启 **dsh web** → 打开工作区（设置里出现「Trivium 记忆」；**芯片**标签默认关）。完整步骤见 [安装](#安装)。
...
然后重启 dsh web。设置里会出现「Trivium 记忆」。芯片标签默认关；若要钉住某些记忆片段，在设置「Trivium 记忆」里打开「芯片标签」后再重启。
```
...
- **芯片（默认关）** — 设置里打开后，标题栏出现 **芯片**；勾选的条目从下一轮注入（L0，≤300 token）。其下还有 **会话层**（也默认关）：把 compaction / 分叉画成方框。
...
- **界面跟随宿主语言** — 设置页和芯片标签随 DSH `locale/change` 切换（`zh` / `en`）。
...
默认关。在设置里打开 **芯片标签**，然后重启 `dsh web`。标题栏出现 **对话 / 轨迹 / 芯片**。这是钉选条，不是记忆编辑器；改名、合并、全局开关仍走设置页。
...
设置里的「Trivium 记忆」管库，也管这两层可选表面。

- **芯片标签（默认关）** — 标题栏「芯片」+ 钉选注入。保存后重启 `dsh web`。
- **会话层（默认关，挂在芯片下）** — 情节画布。要先打开芯片标签。
...
默认关。设置里打开 **芯片** 并重启后，标题栏出现「芯片」。若同时打开 **会话层**，没压过的会话只有一个「后续」方框；顶部是记忆芯片（默认不勾）。可点「生成检查点」把后续收成左边一格。compaction 或 `/compact` 之后左边也会出现历史方框，分叉从方框连到子会话。
...
| 标题栏没有「芯片」 | 正常。默认关。在设置里打开「芯片标签」，保存后重启 `dsh web`。 |
| 改了注入 / 抽取 / embedding / 芯片开关没生效 | 同样要重启 `dsh web`。 |
